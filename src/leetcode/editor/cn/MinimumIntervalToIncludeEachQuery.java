/**
给你一个二维整数数组 intervals ，其中 intervals[i] = [lefti, righti] 表示第 i 个区间开始于 lefti 、结束于 
...
- Query = 22：区间 [20,25] 是包含 22 的最小区间，答案为 25 - 20 + 1 = 6 。
 ...
 Related Topics 数组 二分查找 排序 扫描线 堆（优先队列） 👍 77 👎 0
...
public class MinimumIntervalToIncludeEachQuery {
            ...
            for (int i = 0; i < queries.length; i++) {
                ...
                for (int[] interval : intervals) {
                    if (queries[i] >= interval[0] && queries[i] <= interval[1]) {
                        flag = true;
                        len = Math.min(interval[1] - interval[0] + 1, len);
                    }
                }
                if (!flag) {
                    ans[i] = -1;
    ...
    public static void main(String[] args) {
        ...
        System.out.println(Arrays.toString(solution.minInterval(
