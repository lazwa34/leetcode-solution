...
//</ul>
//
//<div><div>Related Topics</div><div><li>贪心</li><li>数组</li><li>哈希表</li><li>二分查找</li><li>堆（优先队列）</li></div></div><br><div><li>👍 134</li><li>👎 0</li></div>
...
/**
 * @author wangzhiheng
 * @date 2023-10-13 08:27:34
 */
public class AvoidFloodInTheCity {
    //leetcode submit region begin(Prohibit modification and deletion)
    class Solution {
        public int[] avoidFlood(int[] rains) {
            Map<Integer, Integer> rainDays = new HashMap<>();
            TreeSet<Integer> clearDays = new TreeSet<>();
            ...
            for (int i = 0; i < rains.length; i++) {
                if (rains[i] == 0) {
                    ...
                    ans[i] = -1;
                    if (rainDays.containsKey(rains[i])) {
                        Integer clearDay = clearDays.higher(rainDays.get(rains[i]));
                        if (clearDay == null) {
                            return new int[0];
                        }
                        ans[clearDay] = rains[i];
                        clearDays.remove(clearDay);
                    }
                    rainDays.put(rains[i], i);
                }
            }
            return ans;
        }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(
                Arrays.toString(solution.avoidFlood(
                        new int[]{69,0,0,0,69}
                ))
        );
    }
