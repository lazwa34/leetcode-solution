...
输出： [1,2,4]
 ...
 Related Topics 深度优先搜索 广度优先搜索 并查集 数组 矩阵 👍 120 👎 0
...
public class PondSizesLcci {
    ...
    class Solution {
        ...
        public int bfs(int[][] land, int x, int y, int[][] visit) {
            int count = 0;
            LinkedList<Location> queue = new LinkedList<>();
            queue.add(new Location(x, y));
            while (!queue.isEmpty()) {
                Location poll = queue.poll();
                land[poll.x][poll.y] = 1;
                count++;
                for (int[] direction : directions) {
                    int xx = poll.x + direction[0];
                    int yy = poll.y + direction[1];

                    if (xx >= 0 && xx < land.length &&
                            yy >= 0 && yy < land[0].length &&
                            visit[xx][yy] == 0 && land[xx][yy] == 0) {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(solution.pondSizes(new int[][]
